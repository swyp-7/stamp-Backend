...
public class WageServiceImpl implements WageService{
    ...
    public List<WageRes> getEmployeeWage(LocalDate firstDate, UserDetails userDetails) {
        ...
        for (Map.Entry<Long, Duration> entry : timeMap.entrySet()) {
            ...
            Employee employee = employeeRepository.findById(employeeId).orElseThrow(
                    () -> new DomainException(AttendanceErrorCode.EMPLOYEE_ID_ERROR, employeeId.toString())
            );
            ...
            boolean isPaid = isPaidRepository.findByEmployeeAndDate(employee,
                            LocalDate.of(firstDate.getYear(), firstDate.getMonth(), 1))
                    .map(IsPaid::isPaid).orElse(false);


            WageRes wageRes = new WageRes(
                    employeeId,
                    name,
                    totalDuration,
                    hourlyPay,
    ...
    public Void updateEmplyeeWageStatus(WageStatusReq wageStatusReq, LocalDate firstDate) {
        Employee employee = employeeRepository.findById(Long.valueOf(wageStatusReq.employeeId())).orElseThrow();
        IsPaid isPaid = isPaidRepository.findByEmployeeAndDate(employee,
                LocalDate.of(firstDate.getYear(), firstDate.getMonth(), 1)).orElse(
                IsPaid.of(employee, firstDate, wageStatusReq.isPaid())
        );
