...
public class LocalAttendanceServiceImpl implements LocalAttendanceService {

    private final LocalQRManageService qrManageService;
    ...
    /**
     *      Public Method
     *      QR코드 생성 함수. storeId를 받아 QR코드 PNG파일을 Byte List로 반환
     * 권한 : EmployerUser
     * @param storeId   가게 Id
    ...
    /**
     *      Public Method
     *      QR코드 조회 함수. 해당 가게에서 기존에 생성했던 QR코드를 반환한다.
    ...
    public void punchIn(Long storeId, UserDetails userDetails, String authCode){
        ...
        if(attendanceRepository.existsAttendanceByStoreIdAndEmployeeIdAndDateAndAttendance(
            ...
            throw new DomainException(
                    AttendanceErrorCode.DUPLICATE_PUNCH_IN_ERROR,
                    "AttendanceServiceImpl.punchIn");

        attendanceRepository.save(
                Attendance.of(storeId,
                        Long.parseLong(userDetails.getUsername()),
                        AttendanceEnum.PUNCH_IN,
                        LocalDateTime.now()));
    }
    ...
    private void checkEmployerUserAuthority(Long storeId, UserDetails userDetails) {
        ...
        if(!store.getEmployerUser().getId().equals(
