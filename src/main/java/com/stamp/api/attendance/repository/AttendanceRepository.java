...
public interface AttendanceRepository extends JpaRepository<Attendance, String> {
  ...
  /**
   * Attendance의 Id값을 이용하여 출퇴근 로그 조회, AttendanceRes의 형식에 맞춰 반환.
   * Id값을 통해 조회할 로그의 대상 store와 날짜 범위를 조절 가능하다.
   **/
  @Query("SELECT new com.stamp.api.attendance.dto.response.AttendanceRes(a.id, e.name, e.id, a.attendance, a.date, a.time) " +
          "FROM Attendance a " +
          "LEFT JOIN Employee e ON a.storeId = e.id " +
          "WHERE e.id >= :id1 " +
          "AND e.id < :id2")
  List<AttendanceRes> findAttendancesByIdRange(
