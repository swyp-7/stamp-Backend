...
public class EmployeeAttendanceServiceTest {
  ...
  public void setUp() {
    employeeAttendanceService =
        new EmployeeAttendanceServiceImpl(attendanceRepository, employeeRepository);
  }
  ...
  public void getAttendancesForMonth_Success_QueryId_Test() {
    ...
    */
    ...
    verify(attendanceRepository, atLeastOnce())
        .findAttendancesByIdRangeOnEmployee(captor1.capture(), captor2.capture(), anyLong());
    // then
    /* 쿼리 id값의 앞이 storeId를 포함하고 있는지 */
    assertEquals(
        "쿼리 Id값의 앞에 storeId값이 없음",
        String.format("%019d", storeId),
        captor1.getValue().substring(0, 19));

    /* 월 일의 자리수를 제외한 Id값은 동일 */
    assertEquals(
        "쿼리 Id값이 불일치", captor2.getValue().substring(0, 24), captor1.getValue().substring(0, 24));
    assertEquals("쿼리 Id값이 불일치", captor2.getValue().substring(25), captor1.getValue().substring(25));
  ...
  public void getAttendancesForDay_Success_QueryId_Test() {
    ...
    */
    ...
    assertEquals(
        "쿼리 Id값이 불일치", captor2.getValue().substring(0, 26), captor1.getValue().substring(0, 26));
    assertEquals("쿼리 Id값이 불일치", captor2.getValue().substring(27), captor1.getValue().substring(27));
